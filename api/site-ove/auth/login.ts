...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Doit mettre Access-Control-Allow-Credentials: true + origin autorisé
  setCors(req, res);
  ...
  try {
    ...
    // Si tenant non fourni et pas d'OVE_TENANT_ID -> déduire via l'email
    if (!tenantId) {
    ...
    // Déposer aussi un cookie HttpOnly (utile si l'API est sur le même domaine)
    // En cross-site, certains navigateurs peuvent l’ignorer => on renvoie aussi `token` dans le JSON.
    res.setHeader(
      "Set-Cookie",
      `OVE_SESSION=${token}; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=${7 * 24 * 3600}`
      // Optionnel en prod mono-domaine :
      // ; Domain=.ovedistribution.com
    );
    res.setHeader("Cache-Control", "no-store");

    // ✅ Renvoyer le token pour fallback (le front le met en localStorage et l’enverra en Authorization)
