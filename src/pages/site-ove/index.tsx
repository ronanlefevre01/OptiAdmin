  ...
  return (
    <div style={{ padding: 24, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <p style={{ color: '#666', marginBottom: 16 }}>
        Gérez ici les <strong>membres</strong> (accès B2B) et les <strong>commandes</strong> reçues du site.
      </p>

      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button
          onClick={() => setTab('membres')}
          style={btnTabStyle(tab === 'membres')}
        >
          Membres
        </button>
        <button
          onClick={() => setTab('commandes')}
          style={btnTabStyle(tab === 'commandes')}
        >
          Commandes
        </button>
      </div>
...
function MembersPanel() {
  ...
  const [form, setForm] = React.useState<{ email: string; name: string; role: Member['role'] }>({
    email: '',
    name: '',
    role: 'client',
  });
  ...
  React.useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        // Remplace ces URLs par tes vraies routes (Edge/Vercel ou autre)
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          {orders.length === 0 && (
            <div style={{ color: '#667085' }}>Aucune commande pour le moment.</div>
          )}
        </div>
      )}
    </div>
